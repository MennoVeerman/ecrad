...
!             &                              +          yoffs(jcol) *field_in(icol11,jspec) )
  ...
  subroutine diffuse(self, ncol, nspec, ilay, diffusion_length_scale, field_in, field_out)
    ...
    real(jprb), intent(in) :: diffusion_length_scale
    ...
    if ( (max_scale) > sqrt(TwoThirds)) then
      ...
      exchange_factor = diffusion_length_scale / real(n_exchange, jprb)
    else
      n_exchange = 1
